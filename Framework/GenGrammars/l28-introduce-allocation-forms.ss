...
(library (Framework GenGrammars l28-introduce-allocation-forms)
  (export verify-grammar:l28-introduce-allocation-forms)
  ...
  (define verify-grammar:l28-introduce-allocation-forms
    (lambda (x)
      (define Prog
        (lambda (x)
          (match x
      ...
      (let ([res (Prog x)])
        (if res
            (errorf 'verify-grammar:l28-introduce-allocation-forms
              "~a"
              res)
            x)))))
